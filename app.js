...
io.on('connection', (socket) => {
    ...
    socket.on('accept_call', (data) => { io.to("user-" + data.toId).emit('call_accepted'); });
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; outline:none; -webkit-tap-highlight-color: transparent; }
        ...
        .modal { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #1c2128; padding: 40px; border-radius: 25px; border: 1px solid var(--accent); z-index: 2000; text-align: center; display: none; width: 90%; max-width: 320px; box-shadow: 0 0 30px rgba(0,0,0,0.5); }
        .modal-btn { padding: 15px 25px; border-radius: 12px; border: none; color: white; font-weight: bold; cursor: pointer; width: 100%; margin-top: 10px; }
...
<body>
    ...
    <script>
        ...
        if(!userData) {
            userData = {id: Math.floor(100000 + Math.random()*899999), name: "Я"};
            ...
            friends.forEach(f => {
                ...
                d.innerHTML = \`<div>\${isOnline ? '<span class="online-dot"></span>' : ''}<b>\${f.name}</b><br><small>ID: \${f.id}</small></div>
                               <div style="font-size:18px;">
                                 <span onclick="event.stopPropagation(); renameFriend(\${f.id})">✏️</span>
                                 <span onclick="event.stopPropagation(); deleteFriend('\${f.room}')" style="margin-left:10px; opacity:0.5;">🗑️</span>
                               </div>\`;
                list.appendChild(d);
            });
        }
        ...
        function startCall() {
            ...
            if(!f) return;
            callingFriendId = f.id;
            socket.emit('call_user', {toId: f.id, fromName: userData.name});
            document.getElementById('call-status').innerText = "Вызов " + f.name + "...";
            document.getElementById('outgoing-buttons').style.display = 'block';
            document.getElementById('incoming-buttons').style.display = 'none';
            document.getElementById('call-modal').style.display = 'block';
        }

        function cancelCall() {
            socket.emit('cancel_call', {toId: callingFriendId});
            closeCall();
        }

        function rejectCall() {
            socket.emit('decline_call', {toId: callingFriendId});
            closeCall();
        }

        function answerCall() {
            socket.emit('accept_call', {toId: callingFriendId});
            document.getElementById('call-status').innerText = "Соединение...";
            setTimeout(() => { alert("Голосовая связь установлена!"); closeCall(); }, 1000);
        }

        socket.on('incoming_call', (data) => {
            callingFriendId = data.userId; // Кто звонит
            document.getElementById('call-status').innerText = "Звонит " + data.fromName;
            document.getElementById('incoming-buttons').style.display = 'block';
            document.getElementById('outgoing-buttons').style.display = 'none';
            document.getElementById('call-modal').style.display = 'block';
        });

        socket.on('call_declined', () => { alert("Вызов отклонен"); closeCall(); });
        socket.on('call_cancelled', () => { closeCall(); });
        socket.on('call_accepted', () => { alert("Собеседник принял вызов!"); closeCall(); });

        function closeCall() { document.getElementById('call-modal').style.display = 'none'; }
        ...
        socket.on('notify_new_contact', (msg) => {
            if(!friends.find(f => f.id == msg.userId)) saveContact(msg.userId, "Незнакомец #" + msg.userId, msg.room);
        });
