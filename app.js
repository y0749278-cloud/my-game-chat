...
if (fs.existsSync(DB_FILE)) {
    try { db = JSON.parse(fs.readFileSync(DB_FILE, 'utf8')); } catch (e) { console.log("DB Error"); }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('server_auth', (data) => {
        ...
        if (type === 'reg') {
            ...
            db.chats[newUser.id] = []; // Личный список чатов пользователя
            saveDB();
            ...
            if (acc && acc.pass === pass) {
                ...
                socket.emit('sync_chats', db.chats[acc.id] || []); // Отдаем чаты при входе
            } else socket.emit('auth_error', 'Неверные данные!');
        }
    });
    ...
    socket.on('join_room', (room) => { 
        socket.join(room); 
        socket.emit('load_history', db.history.filter(m => m.room === room));
    });

    socket.on('send_msg', (data) => {
        const msg = { id: Date.now() + Math.random(), ...data };
        db.history.push(msg);
        if (db.history.length > 1000) db.history.shift();
        io.to(data.room).emit('new_msg', msg);
        
        // Если это ЛС, сохраняем чат обоим
        if(data.isPrivate) {
            const room = data.room;
            [data.userId, data.toId].forEach(uid => {
                if(!db.chats[uid]) db.chats[uid] = [];
                if(!db.chats[uid].find(c => c.room === room)) {
                    db.chats[uid].push({ name: data.userName, room, type: 'private', tid: data.userId });
                    saveDB();
                }
            });
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; outline: none; -webkit-tap-highlight-color: transparent; margin: 0; padding: 0; }
        ...
        .glass-box { background: var(--panel); padding: 30px; border-radius: 28px; width: 100%; max-width: 320px; border: 1px solid rgba(255,255,255,0.1); text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
        ...
        #rooms-list { flex: 1; overflow-y: auto; padding: 10px; overscroll-behavior: contain; }
        .room-btn { padding: 15px; margin-bottom: 10px; background: rgba(255,255,255,0.03); border-radius: 18px; cursor: pointer; transition: 0.2s; }
        ...
        .msg { max-width: 85%; padding: 12px 18px; border-radius: 22px; font-size: 14px; position: relative; animation: fade 0.2s; }
        ...
        .rec-active { display: none; background: #ee4444; color: white; padding: 8px 15px; border-radius: 20px; font-weight: bold; animation: pulse 1s infinite; }
...
<body>
    ...
    <div id="auth-screen">
        <div class="glass-box">
            <h2 style="color:var(--accent); margin-bottom:20px; font-size:30px;">G-CHAT</h2>
    ...
    <script>
        const socket = io();
        let user = JSON.parse(localStorage.getItem('g_user_v10'));
        let chats = JSON.parse(localStorage.getItem('g_chats_v10')) || [];
        let curRoom = null;
        let recorder, chunks = [];
        ...
        socket.on('auth_success', acc => {
            user = acc;
            localStorage.setItem('g_user_v10', JSON.stringify(user));
            location.reload();
        });

        socket.on('sync_chats', serverChats => {
            chats = serverChats;
            localStorage.setItem('g_chats_v10', JSON.stringify(chats));
            upd();
        });
        ...
        // ГОЛОСОВЫЕ ДЛЯ ANDROID
        const micBtn = document.getElementById('mic-btn');
        const startEvents = ['touchstart', 'mousedown'];
        const stopEvents = ['touchend', 'mouseup'];

        startEvents.forEach(ev => micBtn.addEventListener(ev, e => { e.preventDefault(); startVoice(); }));
        stopEvents.forEach(ev => micBtn.addEventListener(ev, e => { e.preventDefault(); stopVoice(); }));
        ...
        function save(r) { localStorage.setItem('g_chats_v10', JSON.stringify(chats)); switchR(r); }
        ...
        function render(m) {
            ...
            if(m.type==='file') {
                if(m.content.startsWith('data:image')) html = '<img src="'+m.content+'" style="max-width:100%; border-radius:12px; margin-top:5px;">';
                else html = '<a href="'+m.content+'" download="'+m.fileName+'" style="color:#fff; text-decoration:underline;">📄 '+m.fileName+'</a>';
            }
            d.innerHTML = '<div style="font-size:10px; opacity:0.5; margin-bottom:4px;"><b>'+m.userName+'</b></div>' + html;
            b.appendChild(d);
        ...
        socket.on('private_request', d => {
            if(!chats.find(c => c.room === d.room)) {
                const chat = { name: d.fromName, room: d.room, type: 'private', tid: d.fromId };
                chats.push(chat);
                localStorage.setItem('g_chats_v10', JSON.stringify(chats));
                upd();
            }
